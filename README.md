# Hi, I'm Aishat Temitope Kasali 

 Public Health Researcher | Health Data Analyst |  Epidemiologist

I am a Public Health researcher with experience in epidemiology, biostatistics, and health data analysis. I enjoy transforming healthcare data into meaningful insights using reproducible analytical workflows.

My interests include:

- Epidemiology
- Public Health Research
- Health Data Analytics
- Biostatistics
- Reproducible Research

---

## Technical Skills

### Programming

- R
  

### Data Analysis

- tidyverse
- ggplot2
- dplyr
- gtsummary
- broom
- readr
- janitor

### Statistical Analysis

- Descriptive Statistics
- Inferential Statistics
- Logistic Regression
- Data Cleaning
- Exploratory Data Analysis

### Software

- RStudio
- SPSS
- Microsoft Excel
- Git
- GitHub

---

# Featured Projects

## Diabetes 130-US Hospitals Readmission Analysis

An end-to-end analysis of the Diabetes 130-US Hospitals Readmission Dataset (1999–2008) using R.

**Highlights**

- Data import
- Data cleaning
- Data preparation
- Data visualisation
- Descriptive statistics
- Inferential statistics
- Logistic regression
- Publication-ready tables
- Reproducible workflow

🔗 Repository

https://github.com/AishaKs18/diabetes-hospital-readmission-analysis-r

---

## BRFSS 2015 Diabetes Risk Analysis

An epidemiologic analysis of the CDC BRFSS dataset using R.

**Highlights**

- Data cleaning
- Logistic regression
- Model diagnostics
- Statistical reporting
- Git version control

🔗 Repository

https://github.com/AishaKs18/brfss-2015-diabetes-risk-analysis

---

## Education

PhD Public Health

MSc Public Health

BSc Microbiology


## Connect with Me

- GitHub: [AishaKs18](https://github.com/AishaKs18)
- ORCID: https://orcid.org/0009-0009-2337-0518
- ResearchGate: [Aishat Kasali](https://www.researchgate.net/profile/Aishat-Kasali-2)
- LinkedIn: [Aishat Temitope Kasali](www.linkedin.com/in/kasaliaishatemitope)



---


